...
def test_seeds_loaded():
    ...
    assert {"leap", "reverse-string", "two-fer", "isogram", "pangram"}.issubset(ids)
...
def test_get_problem_returns_full_record():
    repo = MockRepository.get_instance()
    p = repo.get_problem("leap")
    assert p is not None
    assert p["function_name"] == "leap_year"
...
def test_record_submission_returns_id():
    repo = MockRepository.get_instance()
    sub_id = repo.record_submission("u1", "leap", True, "def leap_year(y): return True")
...
def test_list_submissions_filters_by_user():
    repo = MockRepository.get_instance()
    repo.record_submission("u1", "leap", True, "code1")
    repo.record_submission("u2", "leap", False, "code2")
    repo.record_submission("u1", "reverse-string", True, "code3")
